...
## Project Structure
.
├── docker-compose.yaml              # Postgres + Kafka + producer + signal_engine + dbt
├── requirements.txt
│
├── # ── Live trading ──────────────────────────────
├── producer.py                      # Alpaca live quotes → Kafka
├── mock_producer.py                 # Replay mock quotes → Kafka
├── mock_signal_test.py              # Synthetic price ramps to test signal engine
├── signal_engine.py                 # Kafka consumer + SMA + regime gate + orders
├── risk_manager.py                  # Position/daily-loss/open-position limits
├── order_executor.py                # Alpaca paper-trading REST wrapper
├── db_logger.py                     # Signal + position persistence
├── stream_quotes.py                 # Live stream helper + mock fixture
├── insert_records.py                # Raw quote archiving
│
├── # ── Data ingestion ───────────────────────────
├── fetch_historical_bars.py         # Daily bars
├── fetch_intraday_bars.py           # 1-min bars
├── historical_fetch.py              # 5-min RTH-only bars
│
├── # ── Feature engineering ──────────────────────
├── feature_engineering.py           # Daily features
├── intraday_feature_engineering.py  # Intraday features (incl. time-of-day volume)
├── intraday_target_engineering.py   # Intraday forward returns
│
├── # ── ML modeling ──────────────────────────────
├── train_model.py                   # Daily direction
├── train_model_regime.py            # Volatility regime
├── cross_sectional.py               # Cross-sectional outperformance
├── train_intraday_model.py          # Intraday direction
├── train_intraday_return_model.py   # Intraday return regression
├── intraday_feature_ablation.py     # Feature-family ablation study
│
├── # ── Backtesting ──────────────────────────────
├── backtest_sma.py                  # Simple SMA crossover
├── orb_vwap_backtest.py             # ORB + VWAP intraday
├── backtest_ml_regime.py            # SMA ± regime gate
├── backtest_regime_gate1.py         # True-holdings walk-forward momentum
├── holdout_test.py                  # Two-phase selection → holdout
├── mullti_window_holdout.py         # Four expanding-window holdout folds
│
└── # ── Results ──────────────────────────────────
    ├── backtest_results.csv
    ├── backtest_summary.csv
    ├── holdout_selection_grid.csv
    ├── holdout_result.csv
    ├── multi_window_summary.csv
    ├── intraday_*_metrics.json
